...
report(void *pvt, FILE *fp, int details)
{
    ...
    if (details > 0) {
        ...
        case 0: fprintf(fp, " -- HiSLIP\n");                break;
        case 1: fprintf(fp, " -- HiSLIP USB488\n");         break;
        default: fprintf(fp, "\n");                         break;
        }
        if (pdpvt->termChar >= 0)
    ...
    for (i = 0 ; i < n ; i++) {
        ...
        for (j = 0 ; j < config->bNumInterfaces ; j++) {
            ...
            for (k = 0 ; k < iface->num_altsetting ; k++) {
                ...
                if ((iface_desc->bInterfaceClass==HiSLIP_INTERFACE_CLASS)
                 && (iface_desc->bInterfaceSubClass==HiSLIP_INTERFACE_SUBCLASS)
                     ...
                     int s, int want, int usbtmcStatus)
{
    if (s < 0) {
        ...
        epicsSnprintf(pasynUser->errorMessage, pasynUser->errorMessageSize,
                            "HiSLIP %s failed: %s", msg, libusb_strerror(s));
        return asynError;
    }
    if (s != want) {
        epicsSnprintf(pasynUser->errorMessage, pasynUser->errorMessageSize,
                    "HiSLIP %s failed -- asked for 0x%x, got %x", msg, want, s);
        return asynError;
    }
    if (usbtmcStatus != 1) {
        const char *cp;
        ...
        epicsSnprintf(pasynUser->errorMessage, pasynUser->errorMessageSize,
            "HiSLIP %s failed -- HiSLIP_status: 0x%x%s", msg, usbtmcStatus, cp);
        return asynError;
    }
    return asynSuccess;
}
...
getCapabilities(drvPvt *pdpvt, asynUser *pasynUser)
{
    ...
    s = libusb_control_transfer(pdpvt->handle,
                ...
                0x07, // bRequest: HiSLIP GET_CAPABILITIES
                0x0000,                  // wValue
...
clearBuffers(drvPvt *pdpvt, asynUser *pasynUser)
{
    ...
    s = libusb_control_transfer(pdpvt->handle,
                0xA1, // bmRequestType: Dir=IN, Type=CLASS, Recipient=INTERFACE
                0x05, // bRequest: HiSLIP INITIATE_CLEAR
                0x0000,                  // wValue
                pdpvt->bInterfaceNumber, // wIndex
                cbuf,                    // data
                1,                       // wLength
    ...
    for (;;) {
        ...
        s = libusb_control_transfer(pdpvt->handle,
                0xA1, // bmRequestType: Dir=IN, Type=CLASS, Recipient=INTERFACE
                0x06, // bRequest: HiSLIP CHECK_CLEAR_STATUS
                0x0000,                  // wValue
                pdpvt->bInterfaceNumber, // wIndex
                cbuf,                    // data
                0x0002,                  // wLength
...
asynInt32Write(void *pvt, asynUser *pasynUser, epicsInt32 value)
{
    ...
    case ASYN_REASON_REN:
        ...
        s = libusb_control_transfer(pdpvt->handle,
                0xA1, // bmRequestType: Dir=IN, Type=CLASS, Recipient=INTERFACE
                160,  // bRequest: HiSLIP REN_CONTROL
                (value != 0),            // wValue
                pdpvt->bInterfaceNumber, // wIndex
                cbuf,                    // data
                1,                       // wLength
...
asynInt32Read(void *pvt, asynUser *pasynUser, epicsInt32 *value)
{
    ...
    case ASYN_REASON_STB:
        ...
        s = libusb_control_transfer(pdpvt->handle,
                0xA1, // bmRequestType: Dir=IN, Type=CLASS, Recipient=INTERFACE
                128,  // bRequest: HiSLIP READ_STATUS_BYTE
                2,                       // wValue (bTag)
                pdpvt->bInterfaceNumber, // wIndex
                cbuf,                    // data
                3,                       // wLength
