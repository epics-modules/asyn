...
namespace nsHiSLIP{
  ...
  public:
    ...
    //   epicsUInt16  protocol_version;
    //   char vendor_id[2]={0x00 ,0x00};
    // } initParm;
    // struct InitializeResponseParameter{
    //   epicsUInt16  session_id;
    //   epicsUInt16  protocol_version;
    // }  initResp;
    message_parameter(epicsUInt32 word){this->word=word;};
    message_parameter(epicsUInt16 proto, char vers[2]){
      this->word= ((epicsInt32)proto << 16) +
	(vers[1]  << 8) + (vers[0]<<0);
      errlogPrintf("messagParameter:%#010x, protocol:%#06x, version:[%c%c]\n",
		   this->word,
		   proto, vers[0], vers[1]);
    };
    message_parameter(epicsUInt16 proto, epicsUInt16 sid){
      this->word = (proto << 16) + sid;
      errlogPrintf("messagParameter:%#010x, protocol:%#06x, session id:%#06x\n",
		   this->word,
		   proto, sid);
    };
    epicsUInt16 getServerProtocolVersion(){
      errlogPrintf("getServerProtocolVersion messagParameter:%#010x, ServerProtocolVersion %d\n",
		   this->word,
		   (epicsUInt16) (((this->word) & 0xffff0000)>>16));
      return (epicsUInt16) (((this->word) & 0xffff0000)>>16);
    }
    epicsUInt16 getSessionId(){
      errlogPrintf("getSessionId messagParameter %#010x, sessionID:%d\n",
		   this->word,
		   (epicsUInt16) ((this->word) & 0xffff));
      return ((epicsUInt16) ((this->word) & 0xffff));
    }
  } message_parameter_t;
  ...
  public:
    ...
    epicsUInt8  message_type;
    epicsUInt8  control_code;
    message_parameter_t message_parameter;
    epicsUInt64 payload_length;
    ...
    Header(const void *buffer):message_parameter(0){
      ...
      this->message_type=*((epicsUInt8 *) ((char *) buffer+2));
      this->control_code=*((epicsUInt8 *) ((char *) buffer+3));
      this->message_parameter.word = be32toh(*(epicsUInt32 *) ((char *) buffer+4));
      this->payload_length = be64toh(*(epicsUInt64 *) ((char *) buffer+8));
    }
    Header(const epicsUInt8  type,
	   const epicsUInt8  cce,
	   const message_parameter_t param,
	   const epicsUInt64 length):message_parameter(param.word){
    ...
    size_t send(int socket){
      ...
      errlogPrintf("sending header dump: %#016qx,%#016qx\n",
		   *((epicsUInt64*)hbuf),*(((epicsUInt64*)hbuf) +1));      
    ...
    int toRawData(void *buffer){ //Serialize this as bytes data in buffer.
      ...
      *((epicsUInt32 *)((char *) buffer+4))=htobe32(this->message_parameter.word);
      *((epicsUInt64 *)((char *) buffer+8))=htobe64(this->payload_length);
      return 0;
    }
  };
  ...
  public:
    ...
    Message(void *raw_header, void *payload):Message(raw_header){
      this->payload= (void *) callocMustSucceed(1,
						this->payload_length,
						"HiSLIP payload buffer");
      memcpy(this->payload, payload, this->payload_length);
    }
    Message(epicsUInt8  type,
	    epicsUInt8  cce,
	    message_parameter_t param,
	    epicsUInt64 length,
	    epicsUInt8 *payload):Header(type,cce,param,length),payload(payload) {
	...
	while (bytestoread){
	  status = ::recv(socket, ((epicsUInt8 *)this->payload+rsize), bytestoread, 0);
  ...
  public:
    ...
    bool rmt_delivered;
    epicsUInt32 message_id;
    epicsUInt32 most_recent_message_id;
    ...
    epicsUInt8 status_query();
    //long write(epicsUInt8 *data_str, long timeout=LOCK_TIMEOUT);
    long write(const epicsUInt8 *data_str, const size_t size, long timeout=LOCK_TIMEOUT);
    size_t ask(epicsUInt8 *data_str, size_t size,
	       epicsUInt8 **rbuffer, long wait_time=LOCK_TIMEOUT);
    int  read(size_t *received, long timeout=LOCK_TIMEOUT );
    int  read(size_t *received, epicsUInt8 **buffer, long timeout=LOCK_TIMEOUT);
    int  read(size_t *received, epicsUInt8 *buffer,  size_t bsize, long timeout=LOCK_TIMEOUT);
  ...
  private:
    ...
    epicsUInt32 increment_message_id(void){
