...
typedef struct devAsynWfPvt{
    ...
    int             nord;
    interruptCallbackInt16Array interruptCallback;
    char            *portName;
    char            *userParam;
    int             addr;
} devAsynWfPvt;
...
static long initCommon(dbCommon *pr, DBLINK *plink, 
    userCallback callback, interruptCallbackInt16Array interruptCallback);
static long processCommon(dbCommon *pr);
static long initWfInt16Array(waveformRecord *pwf);
static long initWfOutInt16Array(waveformRecord *pwf);
...
analogDset asynInt16ArrayWfIn =
    {6, 0, 0, initWfInt16Array,    getIoIntInfo, processCommon, 0};
analogDset asynInt16ArrayWfOut =
    {6, 0, 0, initWfOutInt16Array, getIoIntInfo, processCommon, 0};

epicsExportAddress(dset, asynInt16ArrayWfIn);
epicsExportAddress(dset, asynInt16ArrayWfOut);

static long initCommon(dbCommon *pr, DBLINK *plink, 
    userCallback callback, interruptCallbackInt16Array interruptCallback)
{
    ...
    /* This device support only supports 16-bit integer data types */
    if ((pwf->ftvl != menuFtypeSHORT) && (pwf->ftvl != menuFtypeUSHORT)) {
        errlogPrintf("devAsynWf::initCommon, %s field type must be SHORT or USHORT\n",
                     pr->name);
        goto bad;
    }
    ...
    if(!pasynInterface) {
        errlogPrintf(
            ...
            pr->name, asynInt16ArrayType,pasynUser->errorMessage);
        goto bad;
    }
    pPvt->pint16Array = pasynInterface->pinterface;
    pPvt->int16ArrayPvt = pasynInterface->drvPvt;
...
static long getIoIntInfo(int cmd, dbCommon *pr, IOSCANPVT *iopvt)
{
    ...
    /* If initCommon failed then pPvt->pint16Array is NULL, return error */
    if (!pPvt->pint16Array) return -1;

    if (cmd == 0) {
        /* Add to scan list.  Register interrupts */
        asynPrint(pPvt->pasynUser, ASYN_TRACE_FLOW,
            "%s devAsynInt16Array::getIoIntInfo registering interrupt\n",
            pr->name);
        status = pPvt->pint16Array->registerInterruptUser(
           pPvt->int16ArrayPvt, pPvt->pasynUser,
           pPvt->interruptCallback, pPvt, &pPvt->registrarPvt);
        if(status!=asynSuccess) {
            asynPrint(pPvt->pasynUser, ASYN_TRACE_ERROR,
                      "%s devAsynInt16Array registerInterruptUser %s\n",
                      pr->name, pPvt->pasynUser->errorMessage);
        }
    } else {
        asynPrint(pPvt->pasynUser, ASYN_TRACE_FLOW,
            "%s devAsynInt16Array::getIoIntInfo cancelling interrupt\n",
             pr->name);
        status = pPvt->pint16Array->cancelInterruptUser(pPvt->int16ArrayPvt,
             pPvt->pasynUser, pPvt->registrarPvt);
        if(status!=asynSuccess) {
            asynPrint(pPvt->pasynUser, ASYN_TRACE_ERROR,
                      "%s devAsynInt16Array cancelInterruptUser %s\n",
                      pr->name,pPvt->pasynUser->errorMessage);
        }
    }
...
static void callbackWfOut(asynUser *pasynUser)
{
    ...
    status = pPvt->pint16Array->write(pPvt->int16ArrayPvt,
...
static void callbackWf(asynUser *pasynUser)
{
    ...
    status = pPvt->pint16Array->read(pPvt->int16ArrayPvt,
                ...
                epicsInt16 *value, size_t len)
{
    ...
    dbCommon *pr = pPvt->pr;
    int i;
    epicsInt16 *pint16 = (epicsInt16 *)pwf->bptr;

    asynPrintIO(pPvt->pasynUser, ASYN_TRACEIO_DEVICE,
        (char *)value, len*sizeof(epicsInt16),
        "%s devAsynInt16Array::interruptCallbackInput\n",
        pwf->name);
    if (len > pwf->nelm) len = pwf->nelm;
    for (i=0; i<len; i++) pint16[i] = value[i];
    pPvt->gotValue = 1;
    pPvt->nord = len;
    dbScanLock(pr);
    pr->rset->process(pr);
                ...
                epicsInt16 *value, size_t len)
{
    ...
    asynPrint(pPvt->pasynUser, ASYN_TRACE_ERROR,
        "%s devAsynInt16Array::interruptCallbackOutput not supported yet!\n",
        pr->name);
    /* scanOnce(pr); */
}
