...
typedef struct devAsynWfPvt{
    ...
    int             nord;
    interruptCallbackInt8Array interruptCallback;
    char            *portName;
    char            *userParam;
    int             addr;
} devAsynWfPvt;
...
static long initCommon(dbCommon *pr, DBLINK *plink, 
    userCallback callback, interruptCallbackInt8Array interruptCallback);
static long processCommon(dbCommon *pr);
static long initWfInt8Array(waveformRecord *pwf);
static long initWfOutInt8Array(waveformRecord *pwf);
...
analogDset asynInt8ArrayWfIn =
    {6, 0, 0, initWfInt8Array,    getIoIntInfo, processCommon, 0};
analogDset asynInt8ArrayWfOut =
    {6, 0, 0, initWfOutInt8Array, getIoIntInfo, processCommon, 0};

epicsExportAddress(dset, asynInt8ArrayWfIn);
epicsExportAddress(dset, asynInt8ArrayWfOut);

static long initCommon(dbCommon *pr, DBLINK *plink, 
    userCallback callback, interruptCallbackInt8Array interruptCallback)
{
    ...
    /* This device support only supports 8-bit integer data types */
    if ((pwf->ftvl != menuFtypeCHAR) && (pwf->ftvl != menuFtypeUCHAR)) {
        errlogPrintf("devAsynWf::initCommon, %s field type must be CHAR or UCHAR\n",
                     pr->name);
        goto bad;
    }
    ...
    if(!pasynInterface) {
        errlogPrintf(
            ...
            pr->name, asynInt8ArrayType,pasynUser->errorMessage);
        goto bad;
    }
    pPvt->pint8Array = pasynInterface->pinterface;
    pPvt->int8ArrayPvt = pasynInterface->drvPvt;
...
static long getIoIntInfo(int cmd, dbCommon *pr, IOSCANPVT *iopvt)
{
    ...
    /* If initCommon failed then pPvt->pint8Array is NULL, return error */
    if (!pPvt->pint8Array) return -1;

    if (cmd == 0) {
        /* Add to scan list.  Register interrupts */
        asynPrint(pPvt->pasynUser, ASYN_TRACE_FLOW,
            "%s devAsynInt8Array::getIoIntInfo registering interrupt\n",
            pr->name);
        status = pPvt->pint8Array->registerInterruptUser(
           pPvt->int8ArrayPvt, pPvt->pasynUser,
           pPvt->interruptCallback, pPvt, &pPvt->registrarPvt);
        if(status!=asynSuccess) {
            asynPrint(pPvt->pasynUser, ASYN_TRACE_ERROR,
                      "%s devAsynInt8Array registerInterruptUser %s\n",
                      pr->name, pPvt->pasynUser->errorMessage);
        }
    } else {
        asynPrint(pPvt->pasynUser, ASYN_TRACE_FLOW,
            "%s devAsynInt8Array::getIoIntInfo cancelling interrupt\n",
             pr->name);
        status = pPvt->pint8Array->cancelInterruptUser(pPvt->int8ArrayPvt,
             pPvt->pasynUser, pPvt->registrarPvt);
        if(status!=asynSuccess) {
            asynPrint(pPvt->pasynUser, ASYN_TRACE_ERROR,
                      "%s devAsynInt8Array cancelInterruptUser %s\n",
                      pr->name,pPvt->pasynUser->errorMessage);
        }
    }
...
static void callbackWfOut(asynUser *pasynUser)
{
    ...
    status = pPvt->pint8Array->write(pPvt->int8ArrayPvt,
...
static void callbackWf(asynUser *pasynUser)
{
    ...
    status = pPvt->pint8Array->read(pPvt->int8ArrayPvt,
                ...
                epicsInt8 *value, size_t len)
{
    ...
    dbCommon *pr = pPvt->pr;
    int i;
    epicsInt8 *pint8 = (epicsInt8 *)pwf->bptr;

    asynPrintIO(pPvt->pasynUser, ASYN_TRACEIO_DEVICE,
        (char *)value, len*sizeof(epicsInt8),
        "%s devAsynInt8Array::interruptCallbackInput\n",
        pwf->name);
    if (len > pwf->nelm) len = pwf->nelm;
    for (i=0; i<len; i++) pint8[i] = value[i];
    pPvt->gotValue = 1;
    pPvt->nord = len;
    dbScanLock(pr);
    pr->rset->process(pr);
                ...
                epicsInt8 *value, size_t len)
{
    ...
    asynPrint(pPvt->pasynUser, ASYN_TRACE_ERROR,
        "%s devAsynInt8Array::interruptCallbackOutput not supported yet!\n",
        pr->name);
    /* scanOnce(pr); */
}
