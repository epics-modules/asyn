...
asynStatus paramList::findParam(const char *name, int *index)
{
  ...
  {
    if (name && this->vals[*index].name && (epicsStrCaseCmp(name,
        this->vals[*index].name) == 0))
      return asynSuccess;
  }
...
asynStatus paramList::setInteger(int index, int value)
{
  if (index < 0 || index >= this->nVals)
    return asynParamBadIndex;
  if (this->vals[index].type != asynParamInt32)
...
asynStatus paramList::setUInt32(int index, epicsUInt32 value, epicsUInt32 mask)
{
  if (index < 0 || index >= this->nVals)
    return asynParamBadIndex;
  if (this->vals[index].type != asynParamUInt32Digital)
    return asynParamWrongType;
...
asynStatus paramList::setDouble(int index, double value)
{
  if (index < 0 || index >= this->nVals)
    return asynParamBadIndex;
  if (this->vals[index].type != asynParamFloat64)
    return asynParamWrongType;
...
asynStatus paramList::setString(int index, const char *value)
{
  if (index < 0 || index >= this->nVals)
    return asynParamBadIndex;
  if (this->vals[index].type != asynParamOctet)
    return asynParamWrongType;
...
asynStatus paramList::callCallbacks(int addr)
{
  ...
  for (i = 0; i < this->nFlags; i++)
  {
    ...
    case asynParamUInt32Digital:
      status = uint32Callback(index, addr, this->vals[index].data.uival,
          this->vals[index].uInt32InterruptMask);
      break;
  ...
  for (i = 0; i < this->nVals; i++)
  {
    ...
    case asynParamInt32:
      if (this->vals[i].valueDefined)
        fprintf(fp, "Parameter %d type=asynInt32, name=%s, value=%d\n", i,
            this->vals[i].name, this->vals[i].data.ival);
      else
        fprintf(fp,
            "Parameter %d type=asynInt32, name=%s, value is undefined\n", i,
            this->vals[i].name);
      break;
    case asynParamUInt32Digital:
      if (this->vals[i].valueDefined)
        fprintf(
            fp,
            "Parameter %d type=asynUInt32Digital, name=%s, value=%u, mask=%u\n",
            i, this->vals[i].name, this->vals[i].data.uival,
            this->vals[i].uInt32Mask);
      else
        fprintf(
            fp,
            "Parameter %d type=asynUInt32Digital, name=%s, value is undefined\n",
            i, this->vals[i].name);
      break;
    case asynParamFloat64:
      if (this->vals[i].valueDefined)
        fprintf(fp, "Parameter %d type=asynFloat64, name=%s, value=%f\n", i,
            this->vals[i].name, this->vals[i].data.dval);
      else
        fprintf(fp,
            "Parameter %d type=asynFloat64, name=%s, value is undefined\n", i,
            this->vals[i].name);
      break;
    case asynParamOctet:
      if (this->vals[i].valueDefined)
        fprintf(fp, "Parameter %d type=string, name=%s, value=%s\n", i,
            this->vals[i].name, this->vals[i].data.sval);
      else
    ...
    case asynParamInt8Array:
      if (this->vals[i].valueDefined)
        fprintf(fp, "Parameter %d type=asynInt8Array, name=%s, value=%p\n", i,
            this->vals[i].name, this->vals[i].data.pi8);
      else
        fprintf(fp,
            "Parameter %d type=asynInt8Array, name=%s, value is undefined\n",
            i, this->vals[i].name);
      break;
    case asynParamInt16Array:
      if (this->vals[i].valueDefined)
        fprintf(fp, "Parameter %d type=asynInt16Array, name=%s, value=%p\n", i,
            this->vals[i].name, this->vals[i].data.pi16);
      else
        fprintf(fp,
            "Parameter %d type=asynInt16Array, name=%s, value is undefined\n",
            i, this->vals[i].name);
      break;
    case asynParamInt32Array:
      if (this->vals[i].valueDefined)
        fprintf(fp, "Parameter %d type=asynInt32Array, name=%s, value=%p\n", i,
            this->vals[i].name, this->vals[i].data.pi32);
      else
        fprintf(fp,
            "Parameter %d type=asynInt32Array, name=%s, value is undefined\n",
            i, this->vals[i].name);
      break;
    case asynParamFloat32Array:
      if (this->vals[i].valueDefined)
        ...
        fprintf(
            fp,
            "Parameter %d type=asynFloat32Array, name=%s, value is undefined\n",
            i, this->vals[i].name);
      break;
    case asynParamFloat64Array:
      if (this->vals[i].valueDefined)
        ...
        fprintf(
            fp,
            "Parameter %d type=asynFloat64Array, name=%s, value is undefined\n",
            i, this->vals[i].name);
      break;
    default:
...
asynStatus asynPortDriver::createParam(const char *name, asynParamType type,
    int *index)
{
  int list;
  asynStatus status;
...
void asynPortDriver::reportSetParamErrors(asynStatus status, int index,
    int list, const char *functionName)
{
  if (status == asynParamBadIndex)
  {
...
asynStatus asynPortDriver::setUIntDigitalParam(int index, epicsUInt32 value,
    epicsUInt32 mask)
{
 ...
 * \param[in] value Value to set.
 * \param[in] mask The mask to use when setting the value. */
asynStatus asynPortDriver::setUIntDigitalParam(int list, int index,
 ...
 * \param[in] index The parameter number
 * \param[in] value Address of value to set. */
asynStatus asynPortDriver::setStringParam(int list, int index,
    const char *value)
{
  asynStatus status;
 ...
 * \param[in] list The parameter list number.  Must be < maxAddr passed to asynPortDriver::asynPortDriver.
 * \param[in] functionName The name of the function that generated the error  */
void asynPortDriver::reportGetParamErrors(asynStatus status, int index,
    int list, const char *functionName)
{
  if (status == asynParamBadIndex)
  {
...
asynStatus asynPortDriver::getIntegerParam(int list, int index, int *value)
{
  ...
  if (status)
    reportGetParamErrors(status, index, list, functionName);
  return (status);
}
...
asynStatus asynPortDriver::getUIntDigitalParam(int index, epicsUInt32 *value,
    epicsUInt32 mask)
{
 ...
 * \param[out] value Address of value to get.
 * \param[in] mask The mask to apply when getting the value. */
asynStatus asynPortDriver::getUIntDigitalParam(int list, int index,
    epicsUInt32 *value, epicsUInt32 mask)
{
  asynStatus status;
 ...
 * \param[in] maxChars Maximum number of characters to return.
 * \param[out] value Address of value to get. */
asynStatus asynPortDriver::getStringParam(int list, int index, int maxChars,
    char *value)
{
  asynStatus status;
...
asynStatus readArray(asynUser *pasynUser, epicsType *value, size_t nElements,
    size_t *nIn)
{
...
asynStatus doCallbacksArray(epicsType *value, size_t nElements, int reason,
    int address, void *interruptPvt)
{
  ELLLIST *pclientList;
  interruptNode *pnode;
  int addr;
  ...
  while (pnode)
  {
    ...
    if (addr == -1)
      addr = 0;
    if ((pInterrupt->pasynUser->reason == reason) && (address == addr))
    {
      pInterrupt->callback(pInterrupt->userPvt, pInterrupt->pasynUser, value,
          nElements);
    }
...
void reportInterrupt(FILE *fp, void *interruptPvt,
    const char *interruptTypeString)
{
  ...
  if (interruptPvt)
  {
    ...
    while (pnode)
    {
      ...
      fprintf(
          fp,
          "    %s callback client address=%p, addr=%d, reason=%d, userPvt=%p\n",
...
asynStatus asynPortDriver::getAddress(asynUser *pasynUser, int *address)
{
  ...
  if (*address > this->maxAddr - 1)
  {
    asynPrint(pasynUser, ASYN_TRACE_ERROR,
        ...
        portName, *address, this->maxAddr - 1);
    return (asynError);
  }
  return (asynSuccess);
}
...
asynStatus asynPortDriver::readInt32(asynUser *pasynUser, epicsInt32 *value)
{
  ...
  if (status)
    epicsSnprintf(pasynUser->errorMessage, pasynUser->errorMessageSize,
        "%s:%s: status=%d, function=%d, value=%d", driverName, functionName,
...
extern "C"
{
static asynStatus getBounds(void *drvPvt, asynUser *pasynUser, epicsInt32 *low,
    epicsInt32 *high)
{
  asynPortDriver *pPvt = (asynPortDriver *) drvPvt;
  asynStatus status;
...
asynStatus asynPortDriver::getBounds(asynUser *pasynUser, epicsInt32 *low,
    epicsInt32 *high)
{
  ...
  *low = 0;
  *high = 65535;
  asynPrint(pasynUser, ASYN_TRACEIO_DRIVER, "%s::getBounds,low=%d, high=%d\n",
      driverName, *low, *high);
  return (asynSuccess);
}
    ...
    epicsUInt32 *value, epicsUInt32 mask)
{
  ...
  if (status)
    epicsSnprintf(pasynUser->errorMessage, pasynUser->errorMessageSize,
        "%s:%s: status=%d, function=%d, value=%u mask=%u", driverName,
        functionName, status, function, *value, mask);
  else
...
extern "C"
{
static asynStatus setInterruptUInt32Digital(void *drvPvt, asynUser *pasynUser,
    epicsUInt32 mask, interruptReason reason)
{
  asynPortDriver *pPvt = (asynPortDriver *) drvPvt;
  asynStatus status;
    ...
    epicsUInt32 mask, interruptReason reason)
{
  ...
  if (status)
    epicsSnprintf(pasynUser->errorMessage, pasynUser->errorMessageSize,
        "%s:%s: status=%d, function=%d, mask=%u, reason=%d", driverName,
        functionName, status, function, mask, reason);
  else
    asynPrint(pasynUser, ASYN_TRACEIO_DRIVER,
        "%s:%s: function=%d, mask=%u, reason=%d\n", driverName, functionName,
        function, mask, reason);
  return status;
}

extern "C"
{
static asynStatus clearInterruptUInt32Digital(void *drvPvt,
    asynUser *pasynUser, epicsUInt32 mask)
{
  asynPortDriver *pPvt = (asynPortDriver *) drvPvt;
  asynStatus status;
    ...
    epicsUInt32 mask)
{
  ...
  if (status)
    epicsSnprintf(pasynUser->errorMessage, pasynUser->errorMessageSize,
        "%s:%s: status=%d, function=%d, mask=%u", driverName, functionName,
        status, function, mask);
  else
    asynPrint(pasynUser, ASYN_TRACEIO_DRIVER, "%s:%s: function=%d, mask=%u\n",
...
extern "C"
{
static asynStatus getInterruptUInt32Digital(void *drvPvt, asynUser *pasynUser,
    epicsUInt32 *mask, interruptReason reason)
{
  asynPortDriver *pPvt = (asynPortDriver *) drvPvt;
  asynStatus status;
...
asynStatus asynPortDriver::readFloat64(asynUser *pasynUser, epicsFloat64 *value)
{
  ...
  if (status)
    epicsSnprintf(pasynUser->errorMessage, pasynUser->errorMessageSize,
        "%s:%s: status=%d, function=%d, value=%f", driverName, functionName,
        status, function, *value);
  else
...
extern "C"
{
static asynStatus readOctet(void *drvPvt, asynUser *pasynUser, char *value,
...
asynStatus asynPortDriver::readOctet(asynUser *pasynUser, char *value,
    size_t maxChars, size_t *nActual, int *eomReason)
{
  int function = pasynUser->reason;
  int addr = 0;
...
{
  return (doCallbacksArray<epicsInt8, asynInt8ArrayInterrupt> (value,
      nElements, reason, addr, this->asynStdInterfaces.int8ArrayInterruptPvt));
}
...
asynStatus asynPortDriver::doCallbacksInt16Array(epicsInt16 *value,
    size_t nElements, int reason, int addr)
{
  return (doCallbacksArray<epicsInt16, asynInt16ArrayInterrupt> (value,
      nElements, reason, addr, this->asynStdInterfaces.int16ArrayInterruptPvt));
}
...
asynStatus asynPortDriver::doCallbacksInt32Array(epicsInt32 *value,
    size_t nElements, int reason, int addr)
{
  return (doCallbacksArray<epicsInt32, asynInt32ArrayInterrupt> (value,
      nElements, reason, addr, this->asynStdInterfaces.int32ArrayInterruptPvt));
}
...
asynStatus asynPortDriver::doCallbacksFloat32Array(epicsFloat32 *value,
    size_t nElements, int reason, int addr)
{
  return (doCallbacksArray<epicsFloat32, asynFloat32ArrayInterrupt> (value,
      nElements, reason, addr, this->asynStdInterfaces.float32ArrayInterruptPvt));
}
...
asynStatus asynPortDriver::doCallbacksFloat64Array(epicsFloat64 *value,
    size_t nElements, int reason, int addr)
{
  return (doCallbacksArray<epicsFloat64, asynFloat64ArrayInterrupt> (value,
      nElements, reason, addr, this->asynStdInterfaces.float64ArrayInterruptPvt));
}
...
extern "C"
{
static asynStatus writeGenericPointer(void *drvPvt, asynUser *pasynUser,
    void *genericPointer)
{
  asynPortDriver *pPvt = (asynPortDriver *) drvPvt;
  asynStatus status;
    ...
    int reason, int address)
{
  ...
  pasynManager->interruptStart(
      this->asynStdInterfaces.genericPointerInterruptPvt, &pclientList);
  pnode = (interruptNode *) ellFirst(pclientList);
  while (pnode)
  {
    asynGenericPointerInterrupt *pInterrupt =
        (asynGenericPointerInterrupt *) pnode->drvPvt;
    pasynManager->getAddr(pInterrupt->pasynUser, &addr);
    /* If this is not a multi-device then address is -1, change to 0 */
    if (addr == -1)
      addr = 0;
    ...
    const char *drvInfo, const char **pptypeName, size_t *psize)
{
  ...
  asynPrint(pasynUser, ASYN_TRACE_FLOW, "%s:%s: drvInfo=%s, index=%d\n",
    ...
    const char **pptypeName, size_t *psize)
{
  ...
  asynPrint(pasynUser, ASYN_TRACE_FLOW, "%s:%s: entered", driverName,
      functionName);
...
asynStatus asynPortDriver::drvUserDestroy(asynUser *pasynUser)
{
  ...
  asynPrint(pasynUser, ASYN_TRACE_FLOW, "%s:%s: this=%p, pasynUser=%p\n",
...
void asynPortDriver::report(FILE *fp, int details)
{
  ...
  if (details >= 2)
  {
    ...
    reportInterrupt<asynInt32Interrupt> (fp, pInterfaces->int32InterruptPvt,
        "int32");
    reportInterrupt<asynUInt32DigitalInterrupt> (fp,
        pInterfaces->uInt32DigitalInterruptPvt, "uint32");
    reportInterrupt<asynFloat64Interrupt> (fp,
        pInterfaces->float64InterruptPvt, "float64");
    reportInterrupt<asynOctetInterrupt> (fp, pInterfaces->octetInterruptPvt,
        "octet");
    reportInterrupt<asynInt8ArrayInterrupt> (fp,
        pInterfaces->int8ArrayInterruptPvt, "int8Array");
    reportInterrupt<asynInt16ArrayInterrupt> (fp,
        pInterfaces->int16ArrayInterruptPvt, "int16Array");
    reportInterrupt<asynInt32ArrayInterrupt> (fp,
        pInterfaces->int32ArrayInterruptPvt, "int32Array");
    reportInterrupt<asynFloat32ArrayInterrupt> (fp,
        pInterfaces->float32ArrayInterruptPvt, "float32Array");
    reportInterrupt<asynFloat64ArrayInterrupt> (fp,
        pInterfaces->float64ArrayInterruptPvt, "float64Array");
    reportInterrupt<asynGenericPointerInterrupt> (fp,
        pInterfaces->genericPointerInterruptPvt, "genericPointer");
  }
}
...
{ writeInt32, readInt32, getBounds };

static asynUInt32Digital ifaceUInt32Digital =
{ writeUInt32Digital, readUInt32Digital, setInterruptUInt32Digital,
...
{ writeFloat32Array, readFloat32Array };

static asynFloat64Array ifaceFloat64Array =
{ writeFloat64Array, readFloat64Array };

static asynGenericPointer ifaceGenericPointer =
{ writeGenericPointer, readGenericPointer };
 ...
 */
asynPortDriver::asynPortDriver(const char *portName, int maxAddr,
    int paramTableSize, int interfaceMask, int interruptMask, int asynFlags,
    int autoConnect, int priority, int stackSize)
{
  ...
  status = pasynManager->registerPort(portName, asynFlags, /* multidevice and canblock flags */
  ...
  if (interfaceMask & asynCommonMask)
    pInterfaces->common.pinterface = (void *) &ifaceCommon;
  if (interfaceMask & asynDrvUserMask)
